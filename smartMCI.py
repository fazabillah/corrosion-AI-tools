...
def create_thickness_trend_chart(initial: float, current: float, years: float, rate: float) -> go.Figure:
    ...
    try:
        # Generate trend line
        time_points = [0, years, years + 5, years + 10]
        thickness_points = [
            initial,
            current,
...
def setup_embeddings():
    """Setup embeddings model with multiple fallback options"""
    
    # Option 1: Try HuggingFace embeddings
    if HUGGINGFACE_AVAILABLE:
        try:
            return HuggingFaceEmbeddings(model_name="sentence-transformers/all-mpnet-base-v2")
        except Exception as e:
            st.warning(f"⚠️ HuggingFace embeddings failed: {str(e)[:100]}...")
    
    # Option 2: Try OpenAI embeddings
    try:
        ...
        st.warning(f"⚠️ OpenAI embeddings failed: {str(e)[:100]}...")
    
    # No embeddings available
    st.error("❌ No embedding models available!")
    st.error("Please install: pip install sentence-transformers transformers torch")
    st.error("OR set OPENAI_API_KEY and pip install langchain-openai")
    return None

@st.cache_resource  
def setup_vectorstores():
    """Setup vector stores for API documents with error handling"""
    
    if not PINECONE_AVAILABLE:
        st.error("❌ Pinecone not available. Install with: pip install pinecone-client")
        return {}, False
    ...
    if not embeddings:
        st.error("❌ No embeddings available - cannot setup vector stores")
        return {}, False
    ...
    try:
        ...
        for api_name, index_name in API_INDEXES.items():
            try:
                ...
                st.error(f"❌ Error connecting to {api_name.upper()}: {str(e)[:100]}...")
        
        return vectorstores, len(vectorstores) > 0
        
    except Exception as e:
        st.error(f"❌ Pinecone connection failed: {str(e)[:100]}...")
        return {}, False
